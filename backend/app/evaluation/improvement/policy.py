...
class ImprovementPolicy(ImprovementModel):
    version: Literal["prompt-improvement-policy/1.0"] = "prompt-improvement-policy/1.0"
    ...
    max_total_evaluation_trials: int = Field(default=640, ge=1, le=640)
    ...
    max_total_model_calls: int = Field(default=10, ge=1, le=10)
    max_total_model_tokens: int = Field(default=160_000, ge=1, le=160_000)
    ...
    candidate_sanitizer_version: str = "prompt-candidate-sanitizer/1.1"
    corpus_selection_policy: str = "failure-group-hash-split/1.0"
    screen_selection_policy: str = "target-failures-plus-security-regression/1.0"
    ranking_policy: str = "hard-safety-then-target-class-then-pareto/1.0"
