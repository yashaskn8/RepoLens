"""Groq LLM adapter implementing OpenAI-compatible Chat Completions API."""
...
class GroqAdapter(BaseLLMAdapter):
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        settings = get_settings()
        model = request.model or settings.MODEL_SECURITY_REASONING
        timeout = request.timeout_seconds or settings.LLM_DEFAULT_TIMEOUT

        if not self.api_key:
            raise LLMAuthenticationError(
                "Groq API key is not configured in settings or environment.",
        ...
        try:
            ...
            if not choices:
                raise LLMResponseValidationError("Groq returned empty choices list.", provider=self.provider, model=model)
            ...
            raise LLMResponseValidationError(
                f"Failed to parse Groq response payload: {str(exc)}",
                provider=self.provider,
                model=model,
            )
