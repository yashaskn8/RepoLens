"""Mistral AI LLM adapter implementing Chat Completions API."""
...
class MistralAdapter(BaseLLMAdapter):
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None):
        ...
        self.base_url = (settings.MISTRAL_BASE_URL if base_url is None else base_url).rstrip("/")
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        settings = get_settings()
        model = request.model or settings.MISTRAL_DEFAULT_MODEL
        timeout = request.timeout_seconds or settings.LLM_DEFAULT_TIMEOUT

        if not self.api_key:
            raise LLMAuthenticationError(
                "Mistral API key is not configured in settings or environment.",
        ...
        try:
            ...
            if not choices:
                raise LLMResponseValidationError(
                    "Mistral returned empty choices list.",
                    provider=self.provider,
                    model=model,
                )
            ...
            raise LLMResponseValidationError(
                f"Failed to parse Mistral response: {str(exc)}",
                provider=self.provider,
                model=model,
            ) from exc
